...
|---|---|---|---|
| `packages/parser` | 命令、参数、资源和子场景扫描，保留行信息 | `crabgal-script::ParseReport` | 已有 source span、明确诊断、资源/子场景引用；仍缺发布前独立 check CLI 与不可达分析 |
...
| `setVar` | WebGAL/`.crab`、表达式、数组元素、全局/局部变量和插值 | 对 WebGAL 全部 JS 动态表达式的兼容不作为目标 |
