...
crabgal-core     权威 State、控制流、快照、回滚
        ↓ State
crabgal-bevy     输入、ECS 投影、UI、音频、GPU 合成
```

这种边界让兼容层和运行时不互相污染。WebGAL 的命令可以映射为 crabgal 的领域动作，
而不是迫使引擎复制 WebGAL 的内部 store。`.crab` 也可以在同一核心上提供更严格、
更适合新项目的语法。
...
- 双语法策略：WebGAL 格式负责兼容，`.crab` 负责类型更清晰的新项目体验。
