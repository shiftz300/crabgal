...
| 8 | 4 bytes | little-endian version，当前为 `5` |
...
v5 不尝试兼容旧二进制布局：未知版本由 `inspect` 报告 `Unsupported(version)`，`decode` 直接拒绝。固定时间戳生成的 [`fixtures/store-v5.sav`](../../../crates/loader/src/adapter/store/fixtures/store-v5.sav) 由 `save_v5_golden_is_stable` 逐 byte 比较；只有有意升级格式时才应使用 `CRABGAL_UPDATE_STORE_GOLDEN=1` 重建 fixture，并同步评估是否需要增加版本号。
