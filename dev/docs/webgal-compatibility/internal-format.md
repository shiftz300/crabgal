...
实现位置：[`crates/core/src/model/types.rs`](../../../crates/core/src/model/types.rs)、[`crates/loader/src/adapter/script/webgal/mod.rs`](../../../crates/loader/src/adapter/script/webgal/mod.rs) 和 [`crates/core/src/runtime/step.rs`](../../../crates/core/src/runtime/step.rs)。
...
实现位置：[`src/storage/profile.rs`](../../../src/storage/profile.rs) 和 [`crates/core/src/model/state.rs`](../../../crates/core/src/model/state.rs)。
...
### Save v7 与 `SavedState`

实现位置：[`crates/loader/src/adapter/store/crabgal.rs`](../../../crates/loader/src/adapter/store/crabgal.rs) 和 [`crates/loader/src/adapter/store/mod.rs`](../../../crates/loader/src/adapter/store/mod.rs)。
...
| 8 | 4 bytes | little-endian version，当前为 `6` |
...
v7 不尝试兼容旧二进制布局：未知版本由 `inspect` 报告 `Unsupported(version)`，`decode` 直接拒绝。固定时间戳生成的 [`fixtures/store-v7.sav`](../../../crates/loader/src/adapter/store/fixtures/store-v7.sav) 由 `save_v7_golden_is_stable` 逐 byte 比较；只有有意升级格式时才应使用 `CRABGAL_UPDATE_STORE_GOLDEN=1` 重建 fixture，并同步评估是否需要增加版本号。v7 在既有多粒子状态上加入完整 Godray 相机状态；这次显式升版，避免新字段被旧布局误读。
...
实现位置：[`src/storage/mod.rs`](../../../src/storage/mod.rs)。
