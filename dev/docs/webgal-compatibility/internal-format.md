...
v9 不尝试兼容旧二进制布局：未知版本由 `inspect` 报告 `Unsupported(version)`，`decode` 直接拒绝。固定时间戳生成的 [`store-v9.sav`](../../../crates/loader/tests/fixtures/store-v9.sav) 由 `save_v9_golden_is_stable` 逐 byte 比较；只有有意升级格式时才应使用 `CRABGAL_UPDATE_STORE_GOLDEN=1` 重建 fixture，并同步评估是否需要增加版本号。v9 在 v8 相机后处理状态之上加入可恢复的句尾退格进度和点击等待阶段；舞台时间轴属于瞬态演出，恢复时仍会被清理而不会持久化。这次显式升版，避免新字段被旧布局误读。
