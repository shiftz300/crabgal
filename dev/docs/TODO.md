...
- [x] 独立报告目录 `dev/docs/webgal-compatibility/`：语义矩阵、缺口、视觉边界、内部格式与实测结果
...
- [x] Rust 2024、格式检查、严格 Clippy，并在 Phase 2.5 当时建立 51 项测试质量基线（当前全量为 137 项）
...
- [x] v7 专用二进制存档：元数据前置、状态载荷分离、metadata/state 双 CRC32、只读前缀槽位检查、Program fingerprint、多粒子发射器状态、`SavedState` 恢复边界、固定 golden、原子替换与独立 WebP 预览
...
- [x] SDK 1.0.0 最小同步扩展：只使用 `onRegister`、settings、input action、公开事件与
  `flow.signal`；不访问 DOM、`getHost()`、Electron 或子进程
- [x] `.studio/state.json` 光标到独立 crabgal 窗口的精确单向步进，以及 SDK
  heartbeat/restart 的 loopback 小协议
- [ ] 等待 Studio editor/preview SDK：运行按钮 backend、原版 Player 生命周期、反向 block
  选择与权限受控的工程/进程 API；当前提案见 reference 13
- [ ] 视频和完整相机后处理完成跨平台逐像素/音画验收与 FFmpeg 分发打包
- [ ] SDK 1.x 单文件 ESM 程序扩展、React/Visual UI 与 22 字段 `ExtensionContext`
